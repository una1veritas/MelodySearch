...
struct GeneralMIDI {
	constexpr static struct PROGRAM {
		uint8_t number;
		const char name[40];
	} PROGRAM_NAMES[] = {
			// 	 	Piano
	 ...
	 */
	constexpr static struct PERCUSSION {
		uint8_t number;
		const char name[40];
	} PERCUSSION_NAMES[] = {
			//Hexadecimal
...
struct ScoreElement {
	...
	ScoreElement(const int32_t t, const Event & e, const uint32_t d = 0) :
			time(t), type(NOTE), number(e.notenumber()), velocity(e.velocity()), duration(d) {
		if ( e.isProgChange() ) {
			type = PROGCHANGE;
		} else {
			// note or percussion
